...
const Taskbar: React.FC<TaskbarProps> = ({ onLogout, children }) => {
  return (
    <div className="fixed bottom-0 left-0 right-0 p-4 md:p-6 z-50 pointer-events-none">
      <div className="max-w-4xl mx-auto bg-slate-900/80 backdrop-blur-2xl border border-white/10 px-4 md:px-6 py-3 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex items-center justify-between pointer-events-auto overflow-hidden ring-1 ring-white/5">
        ...
        <button
          onClick={onLogout}
          className="text-slate-400 hover:text-red-400 transition-all p-3 rounded-xl hover:bg-red-500/10 group"
          title="تسجيل الخروج"
        >
